...
function* getList(action) {
  try {
    const result = yield call(() => userService.getAll(ObjectUtil.cleanObjectValue(action.payload)));
...
function* getTotalItem(action) {
  try {
    const data = yield call(() => userService.count(ObjectUtil.cleanObjectValue(action.payload)));
